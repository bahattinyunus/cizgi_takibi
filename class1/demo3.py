...
# Karar sabitleme için geçmiş yönler (noise önleme)
...
def adaptive_threshold(hsv_roi):
    # V kanalı medyanına göre dinamik siyah eşik ayarı
    v = hsv_roi[:, :, 2]
    median_v = np.median(v)
    upper_v = max(50, min(100, median_v + 40))  # adaptif üst sınır (aşırı düşmesini engelle)
    lower = (0, 0, 0)
    upper = (180, 255, upper_v)
    return lower, upper
...
def draw_pid_output(frame, output, position=(30, 100)):
    length = int(np.clip(abs(output)*2, 0, 100))
...
def line_following():
    ...
    while True:
        ...
        frame = cv2.flip(frame, 1)  # Aynala (ayna etkisi)
        ...
        # Adaptive threshold ile maskeleme
        hsv_roi = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
        lower_black, upper_black = adaptive_threshold(hsv_roi)
        mask = cv2.inRange(hsv_roi, lower_black, upper_black)

        # Gürültü azaltma için blur ve morphology
        mask = cv2.GaussianBlur(mask, (5, 5), 0)
        kernel = np.ones((5, 5), np.uint8)
        mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)

        # Kenar tespiti (daha stabil)
        ...
        if contours:
            ...
            if M["m00"] != 0:
                ...
                # Hata bazlı yönlendirme eşikleri
                small_thresh = 20
                medium_thresh = 50
